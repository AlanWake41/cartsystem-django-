...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, "Congratulaions!! Your Account Has Been Activated")
        return redirect('login')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string('accounts/reset_passwrd_email.html', {
                'user': user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),
                'token': default_token_generator.make_token(user),
...
def my_orders(request):
    ...
    orders = Order.objects.filter(user = request.user).order_by('-created_at')#, is_ordered = True
    context = {
        'orders': orders,
    }
